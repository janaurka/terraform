...
type Retention struct {
	...
	Expires int    `json:"expires,string,omitempty"`
...
func resourceRetentionCreate(d *schema.ResourceData, m interface{}) error {
	...
	retention_req, err := client.Post(fmt.Sprintf("v1/retentions"), jsonpayload)
...
func resourceRetentionRead(d *schema.ResourceData, m interface{}) error {
	client := m.(*ShieldClient)
	retention_req, err := client.Get(fmt.Sprintf("v1/retentions"))
	if err != nil {
		return err
	}
